...
public:
     ...
     * @param errorHandler Pointer to error handler (optional, for error statistics)
     */
...
private:
    ...
    CacheErrorHandler* errorHandler_;              // Error handler reference (optional)
    ...
    /**
     * @brief Handle OPC UA connection errors with cache fallback
     * @param nodeId Node ID that failed to read
     * @return ReadResult with cached data or error response
     */
    ReadResult handleOPCConnectionError(const std::string& nodeId);

    /**
     * @brief Build cache-specific error response
     * @param nodeId Node ID that caused the error
     * @param error Error message
     * @param cacheAge Age of cached data (if available)
     * @return HTTP response with cache error information
     */
    crow::response buildCacheErrorResponse(const std::string& nodeId,
