...
public:
    TestableAPIHandler(CacheManager* cacheManager, 
        ...
        : APIHandler(cacheManager, subscriptionManager, opcClient, config) {}
...
protected:
    void SetUp() override {
        ...
        ASSERT_TRUE(subscriptionManager_->initializeSubscription());
...
TEST_F(APIHandlerTest, HandleHealthRequest_ReturnsSystemHealth) {
    ...
    EXPECT_GE(responseJson["active_subscriptions"].get<size_t>(), 0);
...
TEST_F(APIHandlerTest, HandleStatusRequest_ReturnsDetailedStatus) {
    ...
    EXPECT_GE(cache["subscribed_entries"].get<size_t>(), 0);
    EXPECT_GE(cache["total_hits"].get<uint64_t>(), 0);
    EXPECT_GE(cache["total_misses"].get<uint64_t>(), 0);
    
    // Check subscriptions section
    ASSERT_TRUE(responseJson.contains("subscriptions"));
    auto subscriptions = responseJson["subscriptions"];
    EXPECT_GT(subscriptions["subscription_id"].get<uint32_t>(), 0);
    EXPECT_GE(subscriptions["total_monitored_items"].get<size_t>(), 0);
    EXPECT_GE(subscriptions["active_monitored_items"].get<size_t>(), 0);
    EXPECT_TRUE(subscriptions["is_active"]);
